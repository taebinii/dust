...
# 2. 데이터 불러오기 (캐싱을 통해 속도 향상)
@st.cache_data
def load_data():
    # 데이터 불러오기 (한글 깨짐 발생 시 encoding='cp949' 로 변경)
    df = pd.read_csv("서울시 시간별 (초)미세먼지_2025년.csv", encoding="utf-8")
...
# 날짜 범위 선택 (1년치 데이터가 너무 방대할 수 있으므로)
